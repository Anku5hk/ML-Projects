## Data types and structures
  ...
  ### Data Structures(Literal Collections): List, Set, Dict, Tuple.
    * List: Ordered collection of sequence of items, which can be of any literal type. They are Mutable(values can be changed). Indexing is allowed and are iterable.
      Usage:
      ```
       # create list
      ...
      ```
      use as stack: append(value) and pop().
      use as queue: append(value) and pop(0).
      Has methods such as reverse(), sort(), clear(), copy(), index().
      BigO: Indexing and get_length are O(1), all other are O(n).

    * Tuple: Ordered collection of sequence of items. They are Immutable(values cannot be changed), so they are prefered when data should not be changed and so iterating is slightly faster than list.
      ...
      ```
       ...
       my_list[5:] # ['cab',1.0,2.0]
       my_list[:3] # [1,2,3]
       my_list[:] # to create a copy
       
       # list comprehension
       my_list = [x for x in range(10)] # without condition
       my_list = [x for x in range(10) if x > 5] # with condition
       my_list = [True if x > 5 else False for x in range(10)] # also with else condition
      ```
      Has methods count(), index().
      BigO: Indexing and get_length are O(1), all other are O(n).

    * Set: Unordered collection of non repeating sequence of items. Items in set should be immutable. It is iterable, but Indexing/Slicing doesn't work as order does not matter.
      ...
      my_set = set(my_list) # here my_list is mutable, but set() unpacks the items in my_list, but if my_list contained list inside it, TypeError: unhashable type: 'list' is raised.
    ...
    * Dict: Uses Hashtable to store data with key & value. They contain key and values, keys are indexes and values are values to-be/are stored. Dict are very fast and efficient Data structure in Python. Instead if indexing, keys are used to access values.
...
## Classes and Functions
  ...
  * Class: Is a blueprint of object. Which defines what the object holds(which variables/data types), what methods/operations can be performed on that object. 
    ...
    ```
    # define class
    class MyClass: # class with no constructor(python defines automatically in background, no need to define empty constructor)
